"""Synthesize sensor configurations from a trained face_scanning flow.

Loads the flow checkpoint, samples design points, and EM-fits a Gaussian
mixture to produce one or more :class:`~implicitlidar.core.Sensor` per
component count requested. Writes one ``sensors_<n>.csv`` per fit.

Usage::

    python -m implicitlidar.experiments.face_scanning.synthesize \\
...
def main() -> None:
    ...
    parser.add_argument("--checkpoint", type=Path, default=None,
                        help="Defaults to <run_dir>/checkpoints/flow_final.pth.")
    parser.add_argument("--components", type=str, default=None,
                        help="Comma-separated component counts (e.g. '2,4,10').")
